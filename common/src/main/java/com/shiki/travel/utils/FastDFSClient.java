package com.shiki.travel.utils;
...
public class FastDFSClient {
    ...
    public FastDFSClient(String conf) throws Exception {
        if (conf.contains("classpath:")) {
            String path = URLDecoder.decode(getClass().getProtectionDomain().getCodeSource().getLocation().toString(), StandardCharsets.UTF_8);
            path = path.substring(6);
            conf = conf.replace("classpath:", URLDecoder.decode(path, StandardCharsets.UTF_8));
        }
